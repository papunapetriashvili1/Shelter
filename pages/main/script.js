window.addEventListener('DOMContentLoaded', () => {
  ...
  function mainSlider(windowWidth = window.innerWidth) {
    ...
    let slides = slider.querySelectorAll(".slider_item");
    ...
    function nextSlide() {
      if (pageNumber === pagesQuantity && Array.from(Array.from(slides)[slidesQuantity - 1].classList).includes("active_slide")) {
        pageNumber = 1;
        showSlides();
        return
      }
    ...
    function previousSlide() {
      if (pageNumber === 1 && Array.from(Array.from(slides)[0].classList).includes("active_slide")) {
